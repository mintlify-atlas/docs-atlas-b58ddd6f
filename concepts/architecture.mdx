---
title: "Smart Enviro Backend Architecture Overview"
sidebarTitle: "Architecture"
description: "How Smart Enviro Backend connects ESP32 microcontrollers, mobile clients, and the Laravel API server through device tokens, Sanctum bearer tokens, and a MySQL telemetry store."
---
...
| **Redis 7.x** | Cache and queue backend for the Laravel application |
...
<Steps>
  ...
  <Step title="Receive Updated Control State">
    The sync and sensor-data responses both carry a `device_control` payload so the ESP32 always knows the latest commanded state (`ON`, `OFF`, or `STANDBY`) without needing a persistent connection.
  </Step>
</Steps>
